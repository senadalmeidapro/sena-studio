...
# --- Clé d'application ------------------------------------------------
# Une clé éphémère invaliderait les sessions et les données chiffrées à
# chaque redémarrage. Elle doit donc être fournie par l'environnement.
if [ -z "${APP_KEY:-}" ]; then
    echo "==> ÉCHEC : APP_KEY doit être définie dans l'environnement de production"
    exit 1
fi
