<div class="space-y-24 pb-24 sm:space-y-28">
    ...
    <section class="relative overflow-hidden">
        ...
        <div class="relative mx-auto grid max-w-6xl items-center gap-12 px-4 pt-16 pb-4 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:gap-16 lg:px-8 lg:pt-24">
            ...
            <div class="motion-safe:animate-fade-up">
                <span class="inline-flex items-center gap-2 rounded-full border border-emerald-300 bg-emerald-50 px-4 py-1.5 text-xs font-medium text-emerald-700 dark:border-emerald-500/50 dark:bg-emerald-500/10 dark:text-emerald-300">
                ...
                <div class="mt-8 flex flex-wrap items-center gap-3">
                    @foreach (['Laravel', 'Livewire', 'Filament', 'Tailwind'] as $tech)
                        <span class="rounded-full border border-ink-200 bg-white px-3 py-1 text-xs font-medium text-ink-600 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-300">
                            {{ $tech }}
                        </span>
                    @endforeach
                </div>
                ...
                <dl class="mt-12 grid max-w-md grid-cols-3 gap-6 border-t border-ink-200/80 pt-6 dark:border-ink-800/60">
                    <div>
                        ...
                        <dd class="mt-1 text-2xl font-semibold text-emerald-600 dark:text-emerald-300">{{ $this->projectCount }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm text-ink-500 dark:text-ink-400">Expertises</dt>
                        <dd class="mt-1 text-2xl font-semibold text-emerald-600 dark:text-emerald-300">{{ $this->topSkills->count() }}+</dd>
                    </div>
                    <div>
                        <dt class="text-sm text-ink-500 dark:text-ink-400">Sur mesure</dt>
                        <dd class="mt-1 text-2xl font-semibold text-emerald-600 dark:text-emerald-300">100%</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="relative mx-auto w-full max-w-sm motion-safe:animate-fade-up [animation-delay:160ms] lg:max-w-none">
                <div class="relative rotate-2 rounded-[2.25rem] bg-emerald-500 p-1.5 shadow-2xl shadow-emerald-500/25 transition-transform duration-500 hover:rotate-0">
                    <img
                        src="{{ asset('images/portrait.jpeg') }}"
                        alt="Portrait Sena Studio"
                        class="aspect-square w-full rounded-[1.8rem] bg-white object-cover dark:bg-[#0a0a0a]"
                    />
                </div>

                <div class="absolute -left-4 top-8 flex items-center gap-2 rounded-2xl border border-ink-200 bg-white px-4 py-2.5 shadow-lg shadow-ink-900/5 backdrop-blur motion-safe:animate-fade-up [animation-delay:400ms] dark:border-ink-700 dark:bg-[#141414]">
                    <span class="relative flex size-2">
                        <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span>
                        <span class="relative inline-flex size-2 rounded-full bg-emerald-500"></span>
                    </span>
                ...
                <div class="absolute -right-3 bottom-10 rounded-2xl border border-ink-200 bg-white px-4 py-2.5 shadow-lg shadow-ink-900/5 backdrop-blur motion-safe:animate-fade-up [animation-delay:560ms] dark:border-ink-700 dark:bg-[#141414]">
                    <div class="flex items-center gap-1.5 text-xs font-medium text-ink-700 dark:text-ink-200">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-3.5 text-emerald-600 dark:text-emerald-400">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        Web, SaaS &amp; sur mesure
                    </div>
                    <div class="mt-0.5 pl-5 text-[0.7rem] text-ink-500 dark:text-ink-400">Laravel · Livewire · Filament</div>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="mb-12">
            <span class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Ce que je fais</span>
        ...
        <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            @foreach ([
                ...
                <div class="group relative overflow-hidden rounded-2xl border border-ink-200/80 bg-white p-6 shadow-sm shadow-emerald-500/5 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400/60 hover:shadow-lg hover:shadow-emerald-500/10 dark:border-ink-800 dark:bg-ink-900 dark:hover:border-emerald-500/40">
                    <span class="flex size-11 items-center justify-center rounded-xl bg-emerald-500 text-white shadow-md shadow-emerald-500/25 transition-transform duration-300 group-hover:scale-110 dark:bg-emerald-400 dark:text-emerald-950">
    ...
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="mb-12 text-center">
            <span class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Ma méthode</span>
        ...
        <ol class="relative grid gap-6 lg:grid-cols-4">
            <div class="pointer-events-none absolute left-8 right-8 top-7 hidden border-t-2 border-dashed border-emerald-300/70 lg:block dark:border-emerald-700/50" aria-hidden="true"></div>
            ...
            @foreach ([
                ...
                <li class="group relative flex flex-col rounded-2xl border border-ink-200/80 bg-white p-6 pt-8 shadow-sm shadow-emerald-500/5 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400/60 hover:shadow-lg hover:shadow-emerald-500/10 dark:border-ink-800 dark:bg-ink-900 dark:hover:border-emerald-500/40">
                    <div class="flex items-center gap-3">
                        <span class="flex size-11 shrink-0 items-center justify-center rounded-full bg-emerald-500 text-sm font-bold text-white shadow-md shadow-emerald-500/30">
    ...
    @if ($this->featuredProjects->isNotEmpty())
        <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div class="mb-10 flex items-end justify-between">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Portfolio</span>
                ...
                <a href="{{ route('projects.index') }}" wire:navigate class="hidden text-sm font-medium text-emerald-600 transition-colors hover:text-emerald-700 sm:block dark:text-emerald-300 dark:hover:text-emerald-200">
                    Tout voir →
                </a>
            </div>
                    ...
                    <a href="{{ route('projects.show', $project->slug) }}" wire:navigate
                       class="group flex flex-col overflow-hidden rounded-2xl border border-ink-200/80 bg-white shadow-sm shadow-emerald-500/5 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-400/60 hover:shadow-lg hover:shadow-emerald-500/10 dark:border-ink-800 dark:bg-ink-900 dark:hover:border-emerald-500/40">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <h3 class="text-lg font-semibold text-ink-900 transition-colors group-hover:text-emerald-700 dark:text-ink-50 dark:group-hover:text-emerald-300">
                                {{ $project->name }}
                            </h3>
    ...
    @if ($this->topSkills->isNotEmpty())
        <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div class="mb-10 flex items-end justify-between">
                <div>
                    <span class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Compétences</span>
                ...
                <a href="{{ route('skills.index') }}" wire:navigate class="hidden text-sm font-medium text-emerald-600 transition-colors hover:text-emerald-700 sm:block dark:text-emerald-300 dark:hover:text-emerald-200">
            ...
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($this->topSkills as $skill)
                    <a href="{{ route('skills.index') }}" wire:navigate
                       class="group flex items-center justify-between rounded-xl border border-ink-200/80 bg-white px-5 py-4 shadow-sm shadow-emerald-500/5 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-400/60 hover:shadow-md dark:border-ink-800 dark:bg-ink-900 dark:hover:border-emerald-500/40">
                        <span class="flex items-center gap-2.5">
                            @if ($skill->icon)
                                <x-site-icon :icon="$skill->icon" class="size-5" />
                            @endif
                            <span class="font-medium text-ink-800 transition-colors group-hover:text-emerald-700 dark:text-ink-100 dark:group-hover:text-emerald-300">{{ $skill->name }}</span>
                        </span>
    ...
    @if ($this->stackHighlights->isNotEmpty())
        <section class="border-y border-ink-200/80 bg-emerald-50 dark:border-ink-800/60 dark:bg-ink-900">
            <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
                <div class="mb-8 text-center">
                    <span class="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-300">Technologies</span>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                    @foreach ($this->stackHighlights as $category => $items)
                        <div class="rounded-2xl border border-ink-200/80 bg-white p-6 shadow-sm shadow-emerald-500/5 dark:border-ink-800 dark:bg-ink-900">
                            <h3 class="mb-4 text-sm font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-300">{{ ucfirst($category) }}</h3>
                ...
                <div class="mt-8 text-center">
                    <a href="{{ route('stack.index') }}" wire:navigate class="text-sm font-medium text-emerald-600 transition-colors hover:text-emerald-700 dark:text-emerald-300 dark:hover:text-emerald-200">
    ...
    {{-- ===================== CTA ===================== --}}
    <section class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="relative overflow-hidden rounded-3xl bg-emerald-600 p-10 text-center shadow-xl shadow-emerald-700/25 dark:border dark:border-emerald-500/30 sm:p-14">
            ...
            <p class="relative mx-auto mt-3 max-w-xl text-pretty text-emerald-100">
            ...
            <div class="relative mt-8 flex flex-wrap items-center justify-center gap-4">
                <flux:button variant="primary" size="base" :href="route('contact')" wire:navigate class="rounded-xl bg-white text-emerald-800 hover:bg-emerald-50">
                    Démarrer la conversation
                </flux:button>
                <a href="{{ route('stack.index') }}" wire:navigate class="text-sm font-medium text-emerald-200 underline-offset-4 transition-colors hover:text-emerald-100 hover:underline">
