<div class="mx-auto max-w-4xl px-4 pb-24 pt-16 sm:px-6 lg:px-8">
    ...
    <a href="{{ route('projects.index') }}" wire:navigate class="inline-flex items-center gap-1.5 text-sm text-ink-500 transition-colors hover:text-emerald-600 dark:text-ink-400 dark:hover:text-emerald-300">
    ...
    <div class="mt-8 motion-safe:animate-fade-up [animation-delay:80ms]">
        ...
        @if ($galleryUrls->isNotEmpty())
            <div x-data="{ active: 0, images: @js($galleryUrls->all()), count: @js($galleryUrls->count()) }">
                <div class="overflow-hidden rounded-3xl border border-ink-200/80 shadow-lg shadow-emerald-500/10 dark:border-ink-800">
                    <div class="relative aspect-video bg-emerald-100 dark:bg-emerald-900/30">
                    ...
                    <div class="mt-3 flex gap-3">
                        <template x-for="(img, i) in images" :key="i">
                            <button
                                type="button"
                                @click="active = i"
                                :class="i === active ? 'ring-2 ring-emerald-400 ring-offset-2 ring-offset-white dark:ring-offset-ink-950' : 'opacity-70 hover:opacity-100'"
        ...
        <section class="mt-12">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach ($project->skills as $skill)
                    <span class="inline-flex items-center gap-1.5 rounded-lg border border-ink-200 bg-white px-3 py-1.5 text-sm text-ink-700 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-200">
        ...
        <section class="mt-12">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                @foreach ($project->stack->stackItems->groupBy('category') as $category => $items)
                    <div class="rounded-xl border border-ink-200/80 bg-white p-5 shadow-sm shadow-emerald-500/5 dark:border-ink-800 dark:bg-ink-900">
    ...
    @if ($project->infra)
        <section class="mt-12 rounded-2xl border border-ink-200/80 bg-white p-6 shadow-sm shadow-emerald-500/5 dark:border-ink-800 dark:bg-ink-900">
