        ...
        @foreach ($this->byLevel as $levelKey => $skills)
            <section>
                <div class="mb-5 flex items-center gap-3">
                    <span class="flex size-2 rounded-full bg-sage-500 dark:bg-sage-400"></span>
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($skills as $skill)
                        <div class="flex flex-col rounded-2xl border border-ink-200/80 bg-white/70 p-6 shadow-sm shadow-sage-500/5 transition-all duration-300 hover:-translate-y-0.5 hover:border-sage-400/60 hover:shadow-md dark:border-ink-800 dark:bg-ink-900/50 dark:hover:border-sage-500/40">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex items-start gap-3">
                                    @if ($skill->icon)
                                        <span class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-sage-100 dark:bg-sage-500/15">
                            ...
                            <div class="mt-4 flex flex-wrap gap-1.5">
                                @foreach ($skill->projects->take(3) as $project)
                                    <a href="{{ route('projects.show', $project->slug) }}" wire:navigate
                                       class="rounded bg-ink-100/80 px-2 py-0.5 text-xs text-ink-600 transition-colors hover:text-sage-700 dark:bg-ink-800/70 dark:text-ink-300 dark:hover:text-sage-300">
                                        {{ $project->name }}
                                    </a>
                                @endforeach
                            </div>
